...
read.vcf2bed <- function(vcf2bed.in, aps.in=NULL, autosomes.only=T){
  #Read data
  ...
  #Restrict to autosomes only (if optioned)
  if(autosomes.only==T){
...
get.func.indexes.byConstraint <- function(dat, func="LOF", sd_sr.max=1.1, bins=NULL){
  ...
  #Require precise (SR-supported) breakpoints for near-coding functions
  if(func %in% c("INTRONIC", "UTR", "PROMOTER")){
    subdat <- subdat[grep("SR", subdat$EVIDENCE), ]
  }
  if(func=="NEAREST_TSS"){
...
calc.fracSingletons.single <- function(dat, indexes, boot.n=100, conf=0.95, aps=F){
  if(is.na(indexes)){
    point.est <- NA
    ci <- c(NA, NA)
    n.SV <- 0
...
dotplot.fracSingletons.general <- function(singleton.dat, class.colors, class.labels, 
                                           ...
                                           ytck=-0.04, cex.ylabels=0.65, line.ylabel=1.5, aps=F){
...
args <- parse_args(OptionParser(usage="%prog VCF2BED EXON_OVR WHOLEGENE_OVR INV_CENTROMERES INV_RECOMB ALL_RECOMB SD_SR_COV APS_STATS OUTDIR", 
                                option_list=option_list), 
                   positional_arguments=TRUE)
opts <- args$options

###Checks for appropriate positional arguments
if(length(args$args) != 11){
...
# exon_overlap.in <- "~/scratch/gnomAD-SV_v2_rev1.exons_overlapped_per_SV.txt.gz"
# wholegene_overlap.in <- "~/scratch/gnomAD-SV_v2_rev1.whole_genes_overlapped_per_SV.txt.gz"
# inv_centromeres.in <- "~/scratch/gnomAD-SV_v2_rev1.centromeres_per_inversion.txt"
# inv_recomb.in <- "~/scratch/gnomAD-SV_v2_rev1.recombination_hotspots_per_inversion.txt"
# all_recomb.in <- "~/scratch/gnomAD-SV_v2_rev1.recombination_hotspots.all_sv.txt"
# sd_sr_overlap.in <- "~/scratch/gnomAD-SV_v2_rev1.variant_SD_SR_coverage.txt.gz"
# aps.in <- "~/scratch/gnomAD_v2_test_plots/gnomAD-SV_v2_rev1.APS_stats.txt"
...
###Process input data
dat.all <- read.vcf2bed(vcf2bed.in, aps.in)
...
lof.dels.by.exon.count.fracSingles <- rbind(calc.fracSingletons.single(dat=dat, 
                                            ...
                                            calc.fracSingletons.single(dat=dat, 
                                                                       indexes=which(dat$SVTYPE=="DEL" 
                                                                                     ...
                                                                                     & !(dat$chrom %in% c("X", "Y")))))
#Using APS
lof.dels.by.exon.count.aps <- do.call("rbind", lapply(lof.dels.by.exon.count.idxs, 
                                                              calc.fracSingletons.single, 
                                                      dat=dat, aps=T))

lof.dels.by.exon.count.aps <- rbind(calc.fracSingletons.single(dat=dat, 
                                                                       indexes=which(dat$SVTYPE=="DEL" 
                                                                                     & dat$SD_SR_OVERLAP<0.3
                                                                                     & !(dat$chrom %in% c("X", "Y"))),
                                                               aps=T), 
...
lof.dels.by.wholegene.count.fracSingles <- rbind(calc.fracSingletons.single(dat=dat, 
                                                 ...
                                                 calc.fracSingletons.single(dat=dat, 
                                                                            indexes=which(dat$SVTYPE=="DEL" 
                                                                                          ...
                                                                                          & !(dat$chrom %in% c("X", "Y")))))
#Using APS
lof.dels.by.wholegene.count.aps <- do.call("rbind", lapply(lof.dels.by.wholegene.count.idxs, 
                                                                   calc.fracSingletons.single, dat=dat, 
                                                           aps=T))
lof.dels.by.wholegene.count.aps <- rbind(calc.fracSingletons.single(dat=dat, 
                                                                            indexes=which(dat$SVTYPE=="DEL" 
                                                                                          & dat$N_WHOLEGENES==0
                                                                                          & dat$SD_SR_OVERLAP<0.3
                                                                                          & is.na(dat$PROTEIN_CODING__LOF)
                                                                                          & !(dat$chrom %in% c("X", "Y"))),
                                                                    aps=T), 
                                                 lof.dels.by.wholegene.count.aps, 
                                                 calc.fracSingletons.single(dat=dat, 
                                                                            indexes=which(dat$SVTYPE=="DEL" 
                                                                                          & dat$N_WHOLEGENES>2
                                                                                          & dat$SD_SR_OVERLAP<0.3
...
dotplot.fracSingletons.general(singleton.dat=del.plot.dat, 
                               class.colors=rep(svtypes$color[which(svtypes$svtype=="DEL")], 
                                                length(del.plot.dat)), 
                               class.labels=c("All\nDEL", "", 0:8, ">8", "", 0:2, ">2"), 
                               add.counts=F, ytck=-0.02)
# axis(1, at=13.8, labels=">10", tick=F, line=-0.9, cex.axis=0.9)
dev.off()
#Using aps
del.plot.dat.aps <- rbind(lof.dels.by.exon.count.aps, 
                      c(NA, NA, NA, ""), 
                      lof.dels.by.wholegene.count.aps)
pdf(paste(OUTDIR, "/", prefix, ".aps.DEL_by_exon_count.pdf", sep=""), 
    height=2, width=4.75)
dotplot.fracSingletons.general(singleton.dat=del.plot.dat.aps, 
                               class.colors=rep(svtypes$color[which(svtypes$svtype=="DEL")], 
                                                length(del.plot.dat.aps)), 
                               class.labels=c("All\nDEL", "", 0:8, ">8", "", 0:2, ">2"), 
                               add.counts=F, aps=T, ytck=-0.02)
# axis(1, at=13.8, labels=">10", tick=F, line=-0.9, cex.axis=0.9)
dev.off()
...
dotplot.fracSingletons.general(singleton.dat=inv.bycontext.singleton.data, 
                               ...
                               parmar=c(2.25, 2.25, 0.25, 0.25), ytck=-0.02)
dev.off()
#Using APS
inv.bycontext.aps.data <- do.call("rbind",  lapply(inv.bycontext.idx, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
inv.bycontext.aps.data <- rbind(inv.bycontext.aps.data[1, ], 
                                      c(NA, NA, NA, NA), 
                                      inv.bycontext.aps.data[3:4, ], 
                                      c(NA, NA, NA, NA), 
                                      inv.bycontext.aps.data[5:8, ])
pdf(paste(OUTDIR, "/", prefix, ".APS.inv_by_context.pdf", sep=""), 
    height=2, width=2.4)
dotplot.fracSingletons.general(singleton.dat=inv.bycontext.aps.data, 
                               class.colors=rep(svtypes$color[which(svtypes$svtype=="INV")], 
                                                length(inv.bycontext.aps.data)), 
                               class.labels=c("Any\nAny", "", 
                                              "LoF\nAny", "CG\nAny", "", 
                                              "No\n0", "No\n1", "No\n2", "No\n>2"), 
                               add.counts=T, cex.xlabels=0.65, ytck=-0.02, 
                               parmar=c(2.25, 2.25, 0.25, 0.25), aps=T)
dev.off()
...
###########################################################################
###Analyses of SV by functional context stratified by PTV o:e DECILE BY APS
###########################################################################
#pLOF
lof.byconstraint.aps.data <- do.call("rbind",  lapply(lof.indexes.byConstraint, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#IED
ied.byconstraint.aps.data <- do.call("rbind",  lapply(ied.indexes.byConstraint, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#CG
cg.byconstraint.aps.data <- do.call("rbind",  lapply(cg.indexes.byConstraint, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Whole-gene inversion
wginv.byconstraint.aps.data <- do.call("rbind",  lapply(wginv.indexes.byConstraint, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Partial gene duplication
partialdup.byconstraint.aps.data <- do.call("rbind",  lapply(partialdup.indexes.byConstraint, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Intronic
intronic.byconstraint.aps.data <- do.call("rbind",  lapply(intronic.indexes.byConstraint, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Promoter
promoter.byconstraint.aps.data <- do.call("rbind",  lapply(promoter.indexes.byConstraint, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#UTR
utr.byconstraint.aps.data <- do.call("rbind",  lapply(utr.indexes.byConstraint, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Nearest TSS
nearest.byconstraint.aps.data <- do.call("rbind",  lapply(nearest.indexes.byConstraint, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Generate plot
byconstraint.parmar <- c(5, 3, 2, 0.75)
byconstraint.cex.xlabels <- 1
byconstraint.ylims <- c(-0.15, 0.3)
byconstraint.cex.ylabels <- 0.8
byconstraint.line.ylabel <- 2
byconstraint.ytck <- -0.02
pdf(paste(OUTDIR, "/", prefix, ".APS_by_constraint.pdf", sep=""), 
    height=7, width=8)
par(mfrow=c(3, 3))
#pLoF
dotplot.fracSingletons.general(singleton.dat=lof.byconstraint.aps.data, 
                               class.colors=rep(svtypes$color[which(svtypes$svtype=="DEL")], 12), 
                               class.labels=c("ALL", "", 1:10), 
                               add.counts=F, cex.xlabels=byconstraint.cex.xlabels, 
                               parmar=byconstraint.parmar, ylims=byconstraint.ylims, 
                               ytck=byconstraint.ytck, cex.ylabels=byconstraint.cex.ylabels, 
                               line.ylabel=byconstraint.line.ylabel, aps=T)
mtext(3, line=0.1, text="pLoF", font=2)
helper.add.lm(singleton.data=lof.byconstraint.aps.data, 
              color=svtypes$color[which(svtypes$svtype=="DEL")])
#IED
dotplot.fracSingletons.general(singleton.dat=ied.byconstraint.aps.data, 
                               class.colors=rep(svtypes$color[which(svtypes$svtype=="MCNV")], 12), 
                               class.labels=c("ALL", "", 1:10), 
                               add.counts=F, cex.xlabels=byconstraint.cex.xlabels, 
                               parmar=byconstraint.parmar, ylims=byconstraint.ylims, 
                               ...
                               line.ylabel=byconstraint.line.ylabel, aps=T)
mtext(3, line=0.1, text="IED", font=2)
helper.add.lm(singleton.data=ied.byconstraint.aps.data, 
              color=svtypes$color[which(svtypes$svtype=="MCNV")])
#CG
dotplot.fracSingletons.general(singleton.dat=cg.byconstraint.aps.data, 
                               class.colors=rep(svtypes$color[which(svtypes$svtype=="DUP")], 12), 
                               class.labels=c("ALL", "", 1:10), 
                               add.counts=F, cex.xlabels=byconstraint.cex.xlabels, 
                               parmar=byconstraint.parmar, ylims=byconstraint.ylims, 
                               ...
                               line.ylabel=byconstraint.line.ylabel, aps=T)
mtext(3, line=0.1, text="CG", font=2)
helper.add.lm(singleton.data=cg.byconstraint.aps.data, 
              color=svtypes$color[which(svtypes$svtype=="DUP")])
#Whole-gene inversion
dotplot.fracSingletons.general(singleton.dat=wginv.byconstraint.aps.data, 
                               class.colors=rep(svtypes$color[which(svtypes$svtype=="INV")], 12), 
                               class.labels=c("ALL", "", 1:10), 
                               add.counts=F, cex.xlabels=byconstraint.cex.xlabels, 
                               parmar=byconstraint.parmar, ylims=byconstraint.ylims, 
...
mtext(3, line=1.1, text="Whole-Gene INV", font=2)
mtext(3, line=0.1, text="(No Coding Effect)", font=3, cex=0.8)
helper.add.lm(singleton.data=wginv.byconstraint.aps.data, 
              color=svtypes$color[which(svtypes$svtype=="INV")])
#Partial gene duplication
dotplot.fracSingletons.general(singleton.dat=partialdup.byconstraint.aps.data, 
                               class.colors=rep(svtypes$color[which(svtypes$svtype=="DUP")], 12), 
                               class.labels=c("ALL", "", 1:10), 
                               add.counts=F, cex.xlabels=byconstraint.cex.xlabels, 
                               parmar=byconstraint.parmar, ylims=byconstraint.ylims, 
...
mtext(3, line=1.1, text="Partial CG", font=2)
mtext(3, line=0.1, text="(No Coding Effect)", font=3, cex=0.8)
helper.add.lm(singleton.data=partialdup.byconstraint.aps.data, 
              color=svtypes$color[which(svtypes$svtype=="DUP")])
#Intronic
dotplot.fracSingletons.general(singleton.dat=intronic.byconstraint.aps.data, 
                               class.colors=rep("grey35", 12), 
                               class.labels=c("ALL", "", 1:10), 
                               add.counts=F, cex.xlabels=byconstraint.cex.xlabels, 
                               parmar=byconstraint.parmar, ylims=byconstraint.ylims, 
...
mtext(3, line=1.1, text="Intronic", font=2)
mtext(3, line=0.1, text="(No Coding Effect)", font=3, cex=0.8)
helper.add.lm(singleton.data=intronic.byconstraint.aps.data, 
              color="gray35")
#Promoter
dotplot.fracSingletons.general(singleton.dat=promoter.byconstraint.aps.data, 
                               class.colors=rep("grey35", 12), 
                               class.labels=c("ALL", "", 1:10), 
                               add.counts=F, cex.xlabels=byconstraint.cex.xlabels, 
                               parmar=byconstraint.parmar, ylims=byconstraint.ylims, 
...
mtext(3, line=1.1, text="Promoter", font=2)
mtext(3, line=0.1, text="(No Coding Effect)", font=3, cex=0.8)
helper.add.lm(singleton.data=promoter.byconstraint.aps.data, 
              color="gray35")
#UTR
dotplot.fracSingletons.general(singleton.dat=utr.byconstraint.aps.data, 
                               class.colors=rep("grey35", 12), 
                               class.labels=c("ALL", "", 1:10), 
                               add.counts=F, cex.xlabels=byconstraint.cex.xlabels, 
                               parmar=byconstraint.parmar, ylims=byconstraint.ylims, 
...
mtext(3, line=1.1, text="UTR", font=2)
mtext(3, line=0.1, text="(No Coding Effect)", font=3, cex=0.8)
helper.add.lm(singleton.data=utr.byconstraint.aps.data, 
              color="gray35")
#Nearest Gene
dotplot.fracSingletons.general(singleton.dat=nearest.byconstraint.aps.data, 
                               class.colors=rep("grey60", 12), 
                               class.labels=c("ALL", "", 1:10), 
                               add.counts=F, cex.xlabels=byconstraint.cex.xlabels, 
                               parmar=byconstraint.parmar, ylims=byconstraint.ylims, 
...
mtext(3, line=1.1, text="Nearest Gene", font=2)
mtext(3, line=0.1, text="(Strictly Intergenic)", font=3, cex=0.8)
helper.add.lm(singleton.data=nearest.byconstraint.aps.data, 
              color="gray35")
dev.off()
...
###################################################################################
###Analyses of SV by functional context stratified by PTV o:e CLASSIFICATION BY APS
###################################################################################
#pLOF
lof.byConstraintClass.aps.data <- do.call("rbind",  lapply(lof.indexes.byConstraintClass, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#IED
ied.byConstraintClass.aps.data <- do.call("rbind",  lapply(ied.indexes.byConstraintClass, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#CG
cg.byConstraintClass.aps.data <- do.call("rbind",  lapply(cg.indexes.byConstraintClass, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Whole-gene inversion
wginv.byConstraintClass.aps.data <- do.call("rbind",  lapply(wginv.indexes.byConstraintClass, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Partial gene duplication
partialdup.byConstraintClass.aps.data <- do.call("rbind",  lapply(partialdup.indexes.byConstraintClass, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Intronic
intronic.byConstraintClass.aps.data <- do.call("rbind",  lapply(intronic.indexes.byConstraintClass, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Promoter
promoter.byConstraintClass.aps.data <- do.call("rbind",  lapply(promoter.indexes.byConstraintClass, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#UTR
utr.byConstraintClass.aps.data <- do.call("rbind",  lapply(utr.indexes.byConstraintClass, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Nearest TSS
nearest.byConstraintClass.aps.data <- do.call("rbind",  lapply(nearest.indexes.byConstraintClass, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Merge data
aps.data.byConstraintClass <- rbind(lof.byConstraintClass.aps.data, 
                                          c(NA, NA, NA, NA), 
                                          ied.byConstraintClass.aps.data, 
                                          c(NA, NA, NA, NA), 
                                          cg.byConstraintClass.aps.data, 
                                          c(NA, NA, NA, NA), 
                                          partialdup.byConstraintClass.aps.data, 
                                          c(NA, NA, NA, NA), 
                                          intronic.byConstraintClass.aps.data, 
                                          c(NA, NA, NA, NA), 
                                          promoter.byConstraintClass.aps.data, 
                                          c(NA, NA, NA, NA), 
                                          utr.byConstraintClass.aps.data, 
                                          c(NA, NA, NA, NA), 
                                          nearest.byConstraintClass.aps.data)
#Generate plot
pdf(paste(OUTDIR, "/", prefix, ".APS_by_constraint_category.pdf", sep=""), 
    height=2.25, width=8)
dotplot.fracSingletons.general(singleton.dat=aps.data.byConstraintClass, 
                               class.colors=c(rep(svtypes$color[which(svtypes=="DEL")], 4), 
                                              rep(svtypes$color[which(svtypes=="MCNV")], 4), 
                                              rep(svtypes$color[which(svtypes=="DUP")], 4), 
                                              rep(svtypes$color[which(svtypes=="DUP")], 4), 
...
axis(3, tick=F, at=seq(0, 7*7, by=7)+5, line=-0.85, 
     labels=c(rep("", 3), "(Coding Effect Unlikely)" ,  rep("(No Coding Effect)", 3), "(Strictly Intergenic)"), 
...
############################################################################
###Analyses of SV by functional context stratified by PTV o:e SEXTILE BY APS
############################################################################
#pLOF
lof.byConstraintSextile.aps.data <- do.call("rbind",  lapply(lof.indexes.byConstraintSextile, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#IED
ied.byConstraintSextile.aps.data <- do.call("rbind",  lapply(ied.indexes.byConstraintSextile, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#CG
cg.byConstraintSextile.aps.data <- do.call("rbind",  lapply(cg.indexes.byConstraintSextile, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Whole-gene inversion
wginv.byConstraintSextile.aps.data <- do.call("rbind",  lapply(wginv.indexes.byConstraintSextile, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Partial gene duplication
partialdup.byConstraintSextile.aps.data <- do.call("rbind",  lapply(partialdup.indexes.byConstraintSextile, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Intronic
intronic.byConstraintSextile.aps.data <- do.call("rbind",  lapply(intronic.indexes.byConstraintSextile, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Promoter
promoter.byConstraintSextile.aps.data <- do.call("rbind",  lapply(promoter.indexes.byConstraintSextile, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#UTR
utr.byConstraintSextile.aps.data <- do.call("rbind",  lapply(utr.indexes.byConstraintSextile, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#Nearest TSS
nearest.byConstraintSextile.aps.data <- do.call("rbind",  lapply(nearest.indexes.byConstraintSextile, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
#All SV
autosomal.aps.data <- calc.fracSingletons.single(dat, indexes=which(!(dat$chrom %in% c("X", "Y"))), aps=T)
#Merge data
aps.data.byConstraintSextile <- rbind(autosomal.aps.data, 
                                            c(NA, NA, NA, NA), 
                                            lof.byConstraintSextile.aps.data, 
                                            c(NA, NA, NA, NA), 
                                            ied.byConstraintSextile.aps.data, 
                                            c(NA, NA, NA, NA), 
                                            cg.byConstraintSextile.aps.data, 
                                            c(NA, NA, NA, NA), 
                                            partialdup.byConstraintSextile.aps.data, 
                                            c(NA, NA, NA, NA), 
                                            intronic.byConstraintSextile.aps.data, 
                                            c(NA, NA, NA, NA), 
                                            promoter.byConstraintSextile.aps.data, 
                                            c(NA, NA, NA, NA), 
                                            utr.byConstraintSextile.aps.data, 
                                            c(NA, NA, NA, NA), 
                                            nearest.byConstraintSextile.aps.data)
#Generate plot
pdf(paste(OUTDIR, "/", prefix, ".APS_by_constraint_sextile.pdf", sep=""), 
    height=1.05*2, width=1.05*8.75)
dotplot.fracSingletons.general(singleton.dat=aps.data.byConstraintSextile, 
                               class.colors=c("black", NA, 
                                              rep(svtypes$color[which(svtypes=="DEL")], 7), 
                                              rep(svtypes$color[which(svtypes=="MCNV")], 7), 
                                              rep(svtypes$color[which(svtypes=="DUP")], 7), 
                               ...
                               add.counts=T, cex.xlabels=0.75, 
                               parmar=c(1.5, 2.25, 1.75, 0.25), 
                               ytck=-0.02, ylims=c(-0.15, 0.3), aps=T)
axis(3, tick=F, at=seq(0, 7*7, by=7)+5, line=mean(c(-0.2, -0.85)), 
     labels=c("pLoF", "IED", "CG", rep("", 5)), 
     cex.axis=0.85)
axis(3, tick=F, at=seq(0, 7*7, by=7)+5, line=-0.2, 
...
dev.off()
#By APS
cnv.recomb.aps.data <- do.call("rbind",  lapply(cnv.recomb.idx, function(indexes){
  calc.fracSingletons.single(dat=dat, indexes=indexes, aps=T)
}))
del.recomb.aps.plot.data <- rbind(cnv.recomb.aps.data[1, ], 
                                        c(NA, NA, NA, NA), 
                                        cnv.recomb.aps.data[2, ], 
                                        c(NA, NA, NA, NA), 
                                        cnv.recomb.aps.data[3:6, ])
pdf(paste(OUTDIR, "/", prefix, ".APS.del_by_recomb_context.pdf", sep=""), 
    height=2.25, width=2.25)
dotplot.fracSingletons.general(singleton.dat=del.recomb.aps.plot.data, 
                               class.colors=c(rep(svtypes$color[which(svtypes$svtype=="DEL")], 8)), 
                               class.labels=c("Any\nAny", "", 
                                              "LoF\nAny", "", 
                                              "No\n0", "No\n1", "No\n2", "No\n>2"), 
                               add.counts=T, cex.xlabels=0.65, ytck=-0.02, 
                               parmar=c(2.5, 2.25, 0.25, 0.25), aps=T)
dev.off()
dup.recomb.aps.plot.data <- rbind(cnv.recomb.aps.data[7, ], 
                                        c(NA, NA, NA, NA), 
                                        cnv.recomb.aps.data[8:9, ], 
                                        c(NA, NA, NA, NA), 
                                        cnv.recomb.aps.data[10:13, ])
pdf(paste(OUTDIR, "/", prefix, ".APS.dup_by_recomb_context.pdf", sep=""), 
    height=2, width=2.4)
dotplot.fracSingletons.general(singleton.dat=dup.recomb.aps.plot.data, 
                               class.colors=c(rep(svtypes$color[which(svtypes$svtype=="DUP")], 10)), 
                               class.labels=c("Any\nAny", "", 
                                              "IED\nAny", "CG\nAny", "", 
                                              "No\n0", "No\n1", "No\n2", "No\n>2"), 
...
dotplot.fracSingletons.general(singleton.dat=combined.recomb.plot.data, 
                               ...
                               class.labels=c("Any\nAny", "", 
                                              ...
                                              "IED\nAny", "CG\nAny", "", 
                                              "No\n0", "No\n1", "No\n2", "No\n>2"), 
                               add.counts=F, cex.xlabels=0.55, ytck=-0.02,
...
dev.off()
#By APS
combined.recomb.plot.data <- rbind(inv.bycontext.aps.data, 
                                   rep(NA, 4), 
                                   del.recomb.aps.plot.data, 
                                   rep(NA, 4), 
                                   dup.recomb.aps.plot.data)
pdf(paste(OUTDIR, "/", prefix, ".APS.inv_del_dup_by_recomb_context.pdf", sep=""), 
    height=2.5, width=5.6)
dotplot.fracSingletons.general(singleton.dat=combined.recomb.plot.data, 
                               class.colors=c(rep(svtypes$color[which(svtypes$svtype=="INV")], 
                                                  nrow(inv.bycontext.aps.data)+1), 
                                              rep(svtypes$color[which(svtypes$svtype=="DEL")], 
                                                  nrow(del.recomb.aps.plot.data)+1), 
                                              rep(svtypes$color[which(svtypes$svtype=="DUP")], 
                                                  nrow(dup.recomb.aps.plot.data))), 
                               class.labels=c("Any\nAny", "", 
                                              "LoF\nAny", "CG\nAny", "", 
                                              "No\n0", "No\n1", "No\n2", "No\n>2", 
                                              "", 
                               ...
                               add.counts=F, cex.xlabels=0.55, ytck=-0.02,
                               parmar=c(1.75, 2.25, 0.25, 0.25), 
                               dashed.lines=list(c(0, nrow(inv.bycontext.aps.data), 
                                                   inv.bycontext.aps.data[1, 1], 
                                                   svtypes$color[which(svtypes$svtype=="INV")]), 
                                                 c(nrow(inv.bycontext.aps.data)+1, 
                                                   nrow(inv.bycontext.aps.data)+1+nrow(del.recomb.aps.plot.data), 
                                                   del.recomb.aps.plot.data[1, 1], 
                                                   svtypes$color[which(svtypes$svtype=="DEL")]), 
                                                 c(nrow(inv.bycontext.aps.data)+1+nrow(del.recomb.aps.plot.data)+1, 
                                                   nrow(inv.bycontext.aps.data)+1+nrow(del.recomb.aps.plot.data)+1+nrow(dup.recomb.aps.plot.data), 
                                                   dup.recomb.aps.plot.data[1, 1], 
                                                   svtypes$color[which(svtypes$svtype=="DUP")])), 
                               aps=T)
dev.off()

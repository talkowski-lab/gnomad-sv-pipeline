...
plot.singletons <- function(singletons, pops, med.sitesPerSample, max.N, ymax=NULL){
  ...
  mtext(2, text="Singleton SVs per Sample", line=1.7)
...
# singletons.in <- "~/scratch/gnomAD-SV_v2_rev1.downsampling.singletons_per_sample.txt.gz"
# totalsites.in <- "~/scratch/gnomAD-SV_v2_rev1.downsampling.total_sv_sites.txt.gz"
# svpergene.in <- "~/scratch/gnomAD-SV_v2_rev1.downsampling.sv_per_gene.txt.gz"
...
# seed.table.in <- "~/scratch/gnomAD-SV_v2_rev1.downsampling_seeds.txt"
...
plot.fracNonZeroGenes(gene.dat=gene.dat, func="cg", 
               seed.table.in=seed.table.in, 
               color=svtypes$color[which(svtypes$svtype=="DUP")], 
               max.N=max.N, xlabel="Sample Size", ylabel="",
               mar=c(2, 2, 0.3, 0.3))
text(x=par("usr")[1], y=par("usr")[4]-(0.15*(par("usr")[4]-par("usr")[3])), 

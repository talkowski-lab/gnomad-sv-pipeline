...
args <- parse_args(OptionParser(usage="%prog VCF2BED SNV_DATA GENE_METADATA GENELIST_DIRECTORY APS_STATS OUTDIR", 
...
gene.data.noCPX <- getSVdat.all(dat=dat, snv.data=snv.data, include.cpx=F)
gene.data.noCPX.SRonly <- getSVdat.all(dat=dat, snv.data=snv.data, include.cpx=F, require.SR=T)
...
###Get APS per functional class
APSbyFunc <- count.fracSingletons(dat, aps=T)
write.table(APSbyFunc, paste(OUTDIR, "/", prefix, ".APS_by_coding_annotation.txt", sep=""), 
            col.names=T, row.names=F, sep="\t", quote=F)
#Dotplot of singleton fraction
pdf(paste(OUTDIR, "/", prefix, ".APS_by_coding_annotation.pdf", sep=""), 
    height=4.5, width=2.6)
dotplot.fracSingletons(APSbyFunc, aps=T)
dev.off()
pdf(paste(OUTDIR, "/", prefix, ".APS_by_coding_annotation.horizontal.pdf", sep=""), 
    height=2.25, width=4.3)
dotplot.fracSingletons(APSbyFunc, horiz=T, add.pt.labs=F, add.N=F, aps=T)
dev.off()
# pdf(paste(OUTDIR, "/", prefix, ".APS_by_coding_annotation.horizontal.narrow.pdf", sep=""), 
#     height=2.5, width=3.5)
# dotplot.fracSingletons(APSbyFunc, horiz=T, add.pt.labs=F, add.N=F, aps=T)
# dev.off()

# ###Get APS per functional class, no seg dups
# APSbyFunc.noSD <- count.fracSingletons(dat[which(dat$name %in% sd_sr_cov[which(sd_sr_cov[, 2] <= 0.3), 1]), ], aps=T)
# write.table(APSbyFunc.noSD, paste(OUTDIR, "/", prefix, ".APS_by_coding_annotation.noSD.txt", sep=""),
#             col.names=T, row.names=F, sep="\t", quote=F)
# APSbyFunc.wSD <- count.fracSingletons(dat[which(dat$name %in% sd_sr_cov[which(sd_sr_cov[, 2] >= 0.3), 1]), ], aps=T)
# #Dotplot of singleton fraction
# pdf(paste(OUTDIR, "/", prefix, ".APS_by_coding_annotation.noSD.pdf", sep=""),
#     height=4.5, width=2.6)
# dotplot.fracSingletons(APSbyFunc.noSD, aps=T)
# dev.off()
# pdf(paste(OUTDIR, "/", prefix, ".APS_by_coding_annotation.horizontal.noSD.pdf", sep=""),
#     height=2.25, width=4.3)
# dotplot.fracSingletons(APSbyFunc.noSD, horiz=T, add.pt.labs=F, add.N=F, aps=T)
# dev.off()
# dotplot.fracSingletons(APSbyFunc.wSD, horiz=T, add.pt.labs=F, add.N=F, aps=T)
...
###Get APS by SVTYPE for those with coding effects vs intergenic
APS.coding_vs_non <- count.fracSingletons.coding_vs_noncoding(dat, aps=T)
pdf(paste(OUTDIR, "/", prefix, ".APS_by_SVTYPE_matched_coding_vs_non.pdf", sep=""), 
    height=2.25, width=4.9)
dotplot.fracSingletons.coding_vs_non(APS.coding_vs_non, add.N=F, aps=T)
dev.off()
...
constraintModelWrapper(gene.data=gene.data, 
                       ...
                       tck=-0.02, yline=-0.85, parmar=parmar)
dev.off()
#Constraint comparison with SR-only, canonical events
pdf(paste(OUTDIR, "/", prefix, ".snv_constraint_comparison_by_SV_class.all_panels.lof_constraint_only.canonical_SR_only.pdf", sep=""), 
    height=0.45*2.25, width=5.75)
par(mfrow=c(1, 4))
parmar <- c(1.25, 2, 0.5, 0.75)
constraintModelWrapper(gene.data=gene.data.noCPX.SRonly, 
                       y="rare.lof.any", 
                       color=svtypes$color[which(svtypes$svtype=="DEL")], 
                       title="", ymax=1.5, ax.labels=F, cex.labels=0.85, 
                       tck=-0.02, yline=-0.85, parmar=parmar)
...
constraintModelWrapper(gene.data=gene.data.noCPX.SRonly, 
                       ...
                       tck=-0.02, yline=-0.85, parmar=parmar)
dev.off()
pdf(paste(OUTDIR, "/", prefix, ".snv_constraint_comparison_by_SV_class.all_panels.missense_constraint_only.canonical_SR_only.pdf", sep=""), 
    height=0.45*2.25, width=5.75)
par(mfrow=c(1, 4))
parmar <- c(1.25, 2, 0.5, 0.75)
constraintModelWrapper(gene.data=gene.data.noCPX.SRonly, 
...
plot.rareLoFCarrierByList(pop="ALL", dat=dat, 
                          ...
                          genelist.labels=c("Incidentally\nReportable", 
                                            "DDD 76", 
                                            "ClinGen\nHaploinsufficient\n(High Conf.)", 
                                            "ClinGen\nHaploinsufficient\n(Med. Conf.)", 
                                            "ClinGen\nHaploinsufficient\n(Low Conf.)"), 
...
plot.rareLoFCarrierByList.vertical(pop="ALL", dat=dat, 
                                   ...
                                   genelist.labels=c("Incidentally Reportable", 
                                                     "Dominant DD", 
                                                     "ClinGen Haploinsuff.", 
                                                     "Recessive DD", 
                                                     "Recessive DD"), 
...
plot.rareLoFCarrierByList.vertical(pop="ALL", dat=dat, 
                                   ...
                                   genelist.labels=c("Incidental Report.", 
                                                     ...
                                                     "ClinGen Haploinsuff.", 
                                                     "Recessive DD"), 
                                   y.break=c(0.015, 0.05), ymax=0.0775, 
                                   ylabel="Samples", parmar=c(5.9, 3.5, 0.5, 0.5))
dev.off()

# Write table of rare pLoF ACMG variants for manual review
acmg.sv.toreview <- dat[intersect(which(unlist(lapply(strsplit(dat$PROTEIN_CODING__LOF, split=","), function(genes){any(genes %in% ACMG.genes)}))), 
                                  which(dat$AF<0.001)),
                        which(colnames(dat) %in% c("chrom", "start", "end", "name", "SVTYPE", "SVLEN", "ALGORITHMS", 
                                                   "EVIDENCE", "QUAL", "FILTER", "PROTEIN_CODING__LOF", "N_HOMREF", "N_HET", "N_HOMALT"))]
write.table(acmg.sv.toreview,
            paste(OUTDIR, "/", prefix, ".rare_pLoF_SVs_in_ACMG_genes.to_review.txt", sep=""),
            col.names=T, row.names=F, sep="\t", quote=F)


# Comparison of sizes between CG dups and non-CG dups
median(dat$SVLEN[which(dat$SVTYPE=="DUP" & !is.na(dat$PROTEIN_CODING__COPY_GAIN))], na.rm=T)
median(dat$SVLEN[which(dat$SVTYPE=="DUP" & is.na(dat$PROTEIN_CODING__COPY_GAIN))], na.rm=T)
wilcox.test(dat$SVLEN[which(dat$SVTYPE=="DUP" & !is.na(dat$PROTEIN_CODING__COPY_GAIN))],
            dat$SVLEN[which(dat$SVTYPE=="DUP" & is.na(dat$PROTEIN_CODING__COPY_GAIN))],

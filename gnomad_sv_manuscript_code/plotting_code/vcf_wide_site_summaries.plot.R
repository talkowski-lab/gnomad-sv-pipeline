...
plot.freqByContext <- function(dat, svtype, ymin=NULL, axlabel.cex=1,
  ...
  dat <- dat[which(dat$SVTYPE==svtype & !(dat$chrom %in% c("X", "Y"))), ]
  AC.cutoffs <- c(1:9, seq(10, 90, 10), seq(100, 900, 100), seq(1000, 25000, 1000))
  ...
  coding.AFs <- get.af.cdf(dat[which(!is.na(dat$PROTEIN_CODING__LOF)
                                | !is.na(dat$PROTEIN_CODING__COPY_GAIN)
                                | !is.na(dat$PROTEIN_CODING__DUP_LOF)), ],
                           AC.cutoffs)
  coding.nonfunc.AFs <- get.af.cdf(dat[which(is.na(dat$PROTEIN_CODING__LOF)
                                             & is.na(dat$PROTEIN_CODING__COPY_GAIN)
                                             & is.na(dat$PROTEIN_CODING__DUP_LOF)
                                             & (!is.na(dat$PROTEIN_CODING__INTRONIC))
                                             | !is.na(dat$PROTEIN_CODING__DUP_PARTIAL)), ],
                                   AC.cutoffs)
  noncoding.AFs <- get.af.cdf(dat[which(dat$PROTEIN_CODING__INTERGENIC=="True"), ], 
                  AC.cutoffs)
...
args <- parse_args(OptionParser(usage="%prog VCF2BED PRIORSTUDIES POPASSIGNMENTS SD_SR_COV MEDS_PERSAMPLE QUALS APS_STATS OUTDIR", 
...
# vcf2bed.wrelateds.in <- "~/scratch/gnomAD-SV_v2_rev1_wRelateds.vcf2bed.bed.gz"
...
# popassignments.in <- "~/scratch/gnomAD-SV_v2_rev1.updated_pop_assignments.txt"
# sd_sr_coverage.in <- "~/scratch/gnomAD-SV_v2_rev1.variant_SD_SR_coverage.txt.gz"
# medians.perSample.in <- "~/scratch/gnomAD-SV_v2_rev1.median_sites_per_sample_by_population.txt"
...
qualscores <- read.table(quals.in, header=T)
...
sapply(c("DEL", "DUP", "INS", "INV", "CPX"), function(svtype){
  plot.freqByContext(dat=dat, svtype=svtype, ymin=0.45, axlabel.cex=0.8, parmar=c(2.5, 4.25, 2, 0.5))
})
dev.off()
...
plot.fracSingletons.bySize(dat=dat, sd_sr_cov=sd_sr_cov, 
                           svtype="ALL", max.cov=2, include.ins=T, 
...
sapply(c("ALL", "DEL", "DUP", "INS", "INV", "CPX"), function(svtype){
  ...
  plot.fracSingletons.bySize(dat=dat, sd_sr_cov=sd_sr_cov, 
                             svtype=svtype, max.cov=2, include.ins=T, 
                             d=c(-1, 1000, 10000, 100000, 1000000, 1000000000), 
                             d.labels=c("<1kb", "1-10kb", "10-100kb", "100kb-1Mb", ">1Mb"), 
                             color=color, title=title, mar=c(3.5, 2.75, 1.5, 1), 
                             ylab.cex=0.7, ylims=NULL)
})
dev.off()
pdf(paste(OUTDIR, "/", prefix, ".prop_singletons_vs_size.by_svtype.singleRow.pdf", sep=""), 
    height=2, width=7)
par(mfrow=c(1, 6))
sapply(c("ALL", "DEL", "DUP", "INS", "INV", "CPX"), function(svtype){
  if(svtype!="ALL"){
...
dev.off()
pdf(paste(OUTDIR, "/", prefix, ".prop_singletons_vs_size.by_svtype.singleRow.dual.pdf", sep=""), 
    height=2, width=7)
par(mfrow=c(1, 6))
sapply(c("ALL", "DEL", "DUP", "INS", "INV", "CPX"), function(svtype){
  if(svtype!="ALL"){
  ...
  plot.fracSingletons.bySize(dat=dat, sd_sr_cov=sd_sr_cov, 
                             ...
                             d=c(-1, 1000, 10000, 100000, 1000000, 1000000000), 
                             d.labels=c("<1kb", "1-10kb", "10-100kb", "100kb-1Mb", ">1Mb"), 
                             color=color, title=NULL, mar=c(3.5, 2.75, 1.5, 1), 
                             ylab.cex=0.7, ylims=NULL, dual=T)
})
dev.off()

###Plot APS as a function of SV size
pdf(paste(OUTDIR, "/", prefix, ".APS_vs_size.pdf", sep=""), 
    height=2.3, width=2.25)
plot.fracSingletons.bySize(dat=dat, sd_sr_cov=sd_sr_cov, 
                           svtype="ALL", max.cov=2, include.ins=T, 
                           d=c(-1, 1000, 10000, 100000, 1000000, 1000000000), 
                           d.labels=c("<1kb", "1-10kb", "10-100kb", "100kb-1Mb", ">1Mb"), 
                           mar=c(4, 2.75, 0.5, 1), ylab.cex=1, aps=T)
mtext(1, line=3, text="SV Size", cex=1.2)
dev.off()
#Plot grid for all sv classes
pdf(paste(OUTDIR, "/", prefix, ".APS_vs_size.by_svtype.pdf", sep=""), 
    height=2, width=7)
par(mfrow=c(1, 6))
sapply(c("ALL", "DEL", "DUP", "INS", "INV", "CPX"), function(svtype){
  if(svtype!="ALL"){
  ...
  plot.fracSingletons.bySize(dat=dat, sd_sr_cov=sd_sr_cov, 
                             ...
                             d.labels=c("<1kb", "1-10kb", "10-100kb", "100kb-1Mb", ">1Mb"), 
                             color=color, title=title, mar=c(3.5, 2.75, 1.5, 1), 
                             ylab.cex=0.7, ylims=NULL, aps=T)
})
dev.off()
pdf(paste(OUTDIR, "/", prefix, ".APS_vs_size.by_svtype.dual.pdf", sep=""), 
    height=2, width=7)
par(mfrow=c(1, 6))
sapply(c("ALL", "DEL", "DUP", "INS", "INV", "CPX"), function(svtype){
  if(svtype!="ALL"){
  ...
  plot.fracSingletons.bySize(dat=dat, sd_sr_cov=sd_sr_cov, 
                             ...
                             d.labels=c("<1kb", "1-10kb", "10-100kb", "100kb-1Mb", ">1Mb"), 
                             color=color, title=NULL, mar=c(3.5, 2.75, 1.5, 1), 
                             ylab.cex=0.7, ylims=NULL, dual=T, aps=T)
})
dev.off()
...
dev.off()
pdf(paste(OUTDIR, "/", prefix, ".mutation_rate_estimates.no_werling.pdf", sep=""), 
    height=2.9, width=5)
plotMus(dat, Ne=10000, werling=F)
dev.off()

###Chromosomal abnormalities
pdf(paste(OUTDIR, "/", prefix, ".chrom_abnormalities.pdf", sep=""), 
    height=2, width=3)
plot.chrom.abnormalities(dat=dat, cex.toplabels=0.8)
dev.off()
...
###Doubleton analysis
doubleton.stats <- doubleton.analysis(dat, sd_sr_cov)
write.table(doubleton.stats, 
            paste(OUTDIR, "/", prefix, ".doubleton_intrapop_stats_table.txt", sep=""), 
            col.names=T, row.names=F, sep="\t", quote=F)

###QUAL score distributions per SV type, frequency bin, and size
pdf(paste(OUTDIR, "/", prefix, ".qual_score_distribs_multipanel.pdf", sep=""), 
    height=6, width=7)
plot.qualDistribs(qualscores, dat)
dev.off()
